...
Les neuf colonnes de `Transaction` gardent leur nom et leur position. Les colonnes des tables de référence gardent leur nom, sauf en cas de collision : `name` désigne le marchand, colonne attendue telle quelle par le rapport marchand de la Question 4.1, et le nom du produit devient `product_name` ; `category` reste la catégorie de la transaction, sur laquelle la Question 4.1 regroupe ses KPI (elle coïncide avec celle du marchand dans les données fournies, invariant vérifié par `TransformationCheck`), et les catégories de référence deviennent `product_category` et `merchant_category`. Colonnes ajoutées : `transaction_ts`, `transaction_date`, `hour`, `day_of_week`, `month`, `is_weekend`, `day_period`, `is_working_hours` (3.1) ; `user_transaction_rank`, `user_transaction_count`, `age_group` (3.2) ; `amount_last_7_days`, `active_days_last_7_days`, `is_active_user`, `days_since_previous_transaction` (3.3). Le bonus 3.4 (`addSuspiciousFlag`) ajoute `avg_basket_deviation_pct`, `seconds_since_previous_transaction` et `is_suspicious`.
